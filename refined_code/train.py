"""
Main training script for fNIR Base Model.
...
def train(args):
    ...
    # Replace "json" with the config values
    weights_dir = os.path.join(
        args.weights_dir,  # 基础权重目录
        f"{args.model_module}_{args.model_class}_c1to3{args.use_conv1to3}_att{args.use_attention}",  # 添加网络名，例如 "CNNLSTMNet"
        f"cH{args.cnn_input_channels}_{config_values}",  # Use config values or "default" if empty
        dataset_name,
    ...
    for epoch in range(args.num_epochs):
        ...
        # 新一轮测试
        if not args.data_path_88:              print("参数 data_path_88 为空，跳过第二轮测试和保存。\n")
        else:
            # 第二测试集
...
if __name__ == '__main__':
    ...
    # # 你要尝试的多个配置
    # configs = [
    #     {"use_conv1to3": True, "use_attention": True, "cnn_input_channels": 1},
    #     {"use_conv1to3": False, "use_attention": True, "cnn_input_channels": 3},
    #     {"use_conv1to3": True, "use_attention": False, "cnn_input_channels": 1}
    # ]
    #
    # # 数据路径
    # path = r"D:\model_2025_04_01\dataset\npy"
    #
    # # 外层循环：三种模型配置
    # for idx, config in enumerate(configs):
    #     args.use_conv1to3 = config["use_conv1to3"]
    #     args.use_attention = config["use_attention"]
    #     args.cnn_input_channels = config["cnn_input_channels"]
    #
    #     print(f"\n>>> Running Config {idx + 1}: {config}")
    #
    #     # 内层循环：多个数据集子目录
    #     for data_path in os.listdir(path):
    #         args.data_path = os.path.join(path, data_path)
    #         args.metrics_file = f"pro_config{idx + 1}.xlsx"  # 防止写入冲突
