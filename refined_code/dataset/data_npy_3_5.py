"""
data_npy_3_5.py - Part of fNIR Base Model.
"""
    ...
    for sheet_name in sheet_names:
        ...
        for idx in tqdm(range(len(df)), desc=f"Processing {sheet_name}", unit="file"):
            ...
            try:
                ...
                #
                # for frame_idx in range(13):  # 遍历所有13帧
                #     for channel_idx in range(5):  # 遍历所有5通道
                #         ax = axes[frame_idx, channel_idx]
                #         ax.imshow(image[:, :, frame_idx, channel_idx], cmap='gray')
                #         ax.set_xticks([])
                #         ax.set_yticks([])
                #
                #         # 标题设置
                ...
                #         if channel_idx == 0:
                #             ax.set_ylabel(f'Frame {frame_idx + 1}', fontsize=10, rotation=90, labelpad=10)
                #
                # plt.tight_layout()
                # plt.show()
                # (rfdNIR)
                ...
                if image.shape in [(102, 128, 13, 5), (102, 128, 12, 5)]:
                    image = image[..., 1:4]  # 选取第2、3、4个通道，去掉第1和第5个

                # (dNIR)
                if variable_name == 'dNIR':
                    zero_frame = np.zeros((102, 128, 1, 3), dtype=np.float32)
